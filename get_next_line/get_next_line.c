...
/*   Updated: 2022/12/06 15:18:54 by jpark2           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static ssize_t	read_until_nl(t_list *lst, int fd)
{
	...
	if (lst == NULL || lst->str == NULL)
		return (-1);
	rd = read(fd, lst->str, BUFFER_SIZE);
...
static char	*str_concat(char **rm, size_t len, t_list *lst, ssize_t last_rd)
{
	...
	while (lst->next != NULL)
	{
		len += ft_strlcpy(&ret[len], lst->str, BUFFER_SIZE + 1);
		lst = lst->next;
	}
	nl_idx = ft_strnchr(lst->str, '\n', last_rd);
	if (nl_idx == -1)
		nl_idx = last_rd;
	tmp = seperate_nl(rm, lst->str, nl_idx, last_rd);
	if (tmp == NULL)
...
char	*get_next_line(int fd)
{
	...
	if (rms[fd] != NULL)
	{
		r_len = ft_strlen(rms[fd]);
		idx = ft_strnchr(rms[fd], '\n', r_len);
		if (idx != -1)
			return (seperate_nl(&rms[fd], rms[fd], idx, r_len));
	}
	strlst = make_strlst();
	idx = read_until_nl(strlst, fd);
	if (idx == -1)
		return (free_lst(strlst, &rms[fd]));
	ret = str_concat(&rms[fd], r_len, strlst, idx);
	if (ret == NULL)
		free_lst(NULL, &rms[fd]);
	return (ret);
}
